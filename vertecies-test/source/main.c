...
{
  if(buffer_size < sizeof(vertex_list) * num_blocks){
    buffer_size = sizeof(vertex_list) * num_blocks;
    linearFree(vbo_data);
    vbo_data = linearAlloc(buffer_size);
  }

  int row = (int)floor(pow(num_blocks, 1.0/3.0) - 0.5);
  ...
  for(int i = 0;i < num_blocks;i++){
    memcpy(ptr,vertex_list,sizeof(vertex_list));
    vertex * vertices = (vertex*)ptr;
    ptr += sizeof(vertex_list);
    w++;
    if(w > row){
      w = 0;
      h++;
    ...
    for(int j = 0;j < vertex_list_count;j++){
      float r = -(row - 1);
      vertices[j].position[0] += r + (float)w * 2.0;
      vertices[j].position[1] += r + (float)h * 2.0;
      vertices[j].position[2] += r + (float)l * 2.0;
    }
  }
...
static void sceneRender(float iod)
{
  ...
  int row = (int)floor(pow(num_blocks, 1.0/3.0) - 0.5);
	...
	Mtx_Translate(&modelView, 0.0, 0.0, -4.0 + sinf(angleX) + (row * -8.0), true);
  ...
  C3D_DrawArrays(GPU_TRIANGLES, 0, num_blocks * vertex_list_count);
}
